...
Public git repos (URL-only, no creds) + public Helm chart repos. The private
repos (jarvis/brain SSH) are already registered out-of-band
...
# OCI Helm registries (enableOCI: "true"). The actions-runner-controller charts
# (ARC controller + scale set) are published ONLY to the ghcr OCI registry.
HELM_OCI = {
    "actions-runner-controller-charts": "ghcr.io/actions/actions-runner-controller-charts",
}
...
def helm_secret(name, url, oci=False):
    return f"""apiVersion: v1
kind: Secret
metadata:
  name: repo-helm-{name}
...
stringData:
  ...
  name: {name}
  url: {url}
  enableOCI: "{str(oci).lower()}"
"""


docs = []
docs.append("# Declarative ArgoCD repository inventory (managed by the 'root' app-of-apps).")
docs.append("# Public repos: URL-only, no credentials. Private repos (jarvis/brain)")
...
parts += [helm_secret(n, u, oci=True) for n, u in HELM_OCI.items()]

output_path = Path(__file__).with_name("repositories.yaml")
with output_path.open("w") as fh:
    fh.write("---\n".join(p.rstrip() + "\n" for p in parts))

print(f"wrote {len(GIT_PUBLIC)} git + {len(HELM_PUBLIC)} helm + {len(HELM_OCI)} oci repository secrets")
